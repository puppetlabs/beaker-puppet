      ...
      module FOSSUtils
        ...
        def install_puppet_agent_on(hosts, opts = {})
          ...
          block_on hosts, { :run_in_parallel => run_in_parallel } do |host|
            ...
            package_name = nil
            case host['platform']
            when /el-|rhel|redhat|fedora|sles|centos|cisco_/
        ...
        def install_puppetlabs_release_repo_on( hosts, repo = nil, opts = options )
          block_on hosts do |host|
            ...
            case variant
            when /^(fedora|el|redhat|rhel|centos|sles|cisco_nexus|cisco_ios_xr)$/
              variant_url_value = ((['redhat','rhel','centos'].include?($1)) ? 'el' : $1)
        ...
        def install_puppetlabs_dev_repo ( host, package_name, build_version,
                                  ...
                                  opts = options )
          variant, version, arch, codename = host['platform'].to_array
          if variant !~ /^(fedora|el|rhel|redhat|centos|debian|ubuntu|cumulus|huaweios|cisco_nexus|cisco_ios_xr|sles)$/
            raise "No repository installation step for #{variant} yet..."
          end
          ...
          dev_builds_url = opts[:dev_builds_url] if variant =~ /^(fedora|el|rhel|redhat|centos)$/
        ...
        def install_packages_from_local_dev_repo( host, package_name )
          if host['platform'] =~ /debian|ubuntu|cumulus|huaweios/
            find_filename = '*.deb'
            find_command  = 'dpkg -i'
          elsif host['platform'] =~ /fedora|el|rhel|redhat|centos/
            find_filename = '*.rpm'
        ...
        def install_puppet_agent_dev_repo_on( hosts, global_opts )
          ...
          block_on hosts do |host|
            ...
            case variant
            when /^(fedora|el|rhel|redhat|centos|debian|ubuntu|cumulus|huaweios|cisco_nexus|cisco_ios_xr)$/
            ...
            when /^(sles|aix|fedora|el|rhel|redhat|centos)$/
        ...
        def remove_puppet_on( hosts )
          block_on hosts do |host|
            ...
            when /aix|sles|el|rhel|redhat|centos|oracle|scientific/
